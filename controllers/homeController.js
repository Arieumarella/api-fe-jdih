...
exports.getKurvaPengunjung = async (req, res) => {
  try {
    const qry =
      "SELECT  CASE DAYOFWEEK(waktu) WHEN 1 THEN 'Minggu' WHEN 2 THEN 'Senin' WHEN 3 THEN 'Selasa' WHEN 4 THEN 'Rabu' WHEN 5 THEN 'Kamis' WHEN 6 THEN 'Jumat' WHEN 7 THEN 'Sabtu' END AS name, jml as Pengunjung FROM tb_hit_date WHERE waktu BETWEEN CURDATE() - INTERVAL 7 DAY AND CURDATE() ORDER BY waktu asc";
    const data = await prisma.$queryRawUnsafe(qry);
...
exports.getJenisPeraturan = async (req, res) => {
  try {
    const qry =
      "select peraturan_category_id,percategoryname, singkatan_file from ppj_peraturan_category where percategorykondisi='1'  and tipe='1' order by ppj_peraturan_category.order ASC";
...
exports.getDataBerita = async (req, res) => {
  try {
    const qry =
      "select id, judul, CONCAT('https://jdih.pu.go.id/internal/assets/assets/berita/', gambar_1) AS path_file, slug from tb_berita order by tgl_modifikasi DESC limit 7";
    const data = await prisma.$queryRawUnsafe(qry);
